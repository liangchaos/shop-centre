...
public class ShopServiceApplication {
    ...
    private static Logger logger = LoggerFactory.getLogger (ShopServiceApplication.class);

    public static void main (String[] args) {
        Date startDate = new Date ();

        SpringApplicationBuilder springApp = new SpringApplicationBuilder (ShopServiceApplication.class);
        springApp.web (WebApplicationType.SERVLET);
        springApp.run (args);

        UserBannerPrinter bannerPrinter = new UserBannerPrinter (startDate);
        bannerPrinter.printBanner (springApp.context ().getEnvironment (), ShopServiceApplication.class, System.out);
